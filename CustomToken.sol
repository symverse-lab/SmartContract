...
contract ExchangeSYM is Ownable {
    ...
    /*
     * 교환을 요청하는 함수
     * 토큰 계약의 exchange 함수는 외부 호출만 가능하며,
     * 호출한 대상이 Delegator로 지정된 계약만 가능하므로 별도의 실행 함수가 필요함.
     */
    function exchangeSYM() external returns (bool) {
        (bool result,) = _tokenContract.call(abi.encodeWithSignature("exchange(address)", msg.sender));
        return result;
    }
    
    /*
     * 교환을 위해 Delegator가 대신 받았던 모든 금액을 토큰 계약의 Owner한테 돌려준다.
     */
    function withdraw() external onlyOwner returns (bool) {
        (bool result,) = _tokenContract.call(abi.encodeWithSignature("withdraw()"));
        return result;
    }
    
    /*
     * 교환을 진행하면서 회수된 금액을 확인하는 함수
     */
    function balance() external onlyOwner returns (uint256) {
        (, bytes memory balanceData) = _tokenContract.call(abi.encodeWithSignature("balanceOf(address)", address(this)));
        (uint256 balanceValue) = abi.decode(balanceData, (uint256));
        return balanceValue;
    }
}
